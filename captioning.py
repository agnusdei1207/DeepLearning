...
Original file is located at
    https://colab.research.google.com/drive/1Q1HA89e8zDI0RSxSvhYpvt6Vv5qQdZ-G
...
if COLAB:
    root_captioning = "/content/drive/MyDrive/Colab Notebooks/data/ImageCaptioning"
else:
    root_captioning = "./data/ImageCaptioning"

# Commented out IPython magic to ensure Python compatibility.
!pwd
# %cd /content/drive/MyDrive/Colab Notebooks/data/ImageCaptioning

# -d 해당 파일을 생성하여 저장해라
!unzip ./Flickr8k_Dataset.zip -d ./Flickr_image

!unzip ./Flickr8k_text.zip -d ./Flickr_text
...
# translate 라는 함수를 null_punct 에 담는다
null_punct = str.maketrans('', '', string.punctuation) # 따옴표, 마침표, 물음표 제거 
lookup = dict() # 이미지와 해당 이미지에 맞는 문자를 묶어주는 딕셔너리

with open( os.path.join(root_captioning,'/content/drive/MyDrive/Colab Notebooks/data/ImageCaptioning/Flickr_text/Flickr8k.token.txt',\
                        '/content/drive/MyDrive/Colab Notebooks/data/ImageCaptioning/Flickr_text/Flickr8k.token.txt'), 'r') as fp:
  
  # fp : 모든 문자열을 읽는다, 개행을 기준
  max_length = 0
  for line in fp.read().split('\n'):
    tok = line.split()
    if len(line) >= 2:
      id = tok[0].split('.')[0] # 확장자를 제거한 이미지 이름만 가져온다.
      desc = tok[1:] # 해당 이미지 설명 글 추출
      
      # Cleanup description
      # desc : 사진을 설명하는 문장
      desc = [word.lower() for word in desc] # 소문자로 변환
      desc = [w.translate(null_punct) for w in desc] # 마침표, 물음표 등등 제거
      desc = [word for word in desc if len(word)>1]  # 2글자 이상만 단어로 인식하도록 추출
      desc = [word for word in desc if word.isalpha()] # 알파벳만 남겨 놓기
      max_length = max(max_length,len(desc)) # 문장을 구성하는 단어의 개수가 가장 많은 숫자 저장
      
      # 사진 한 장당 5문장으로 표현되어 있는 데이터셋
      # id : 사진 이미지 이름
      # 사진의 이름을 key값으로 value 는 list로 집어 넣기
      if id not in lookup: 
        lookup[id] = list() # 사진이 같다면 리스트에 추가하기 
      lookup[id].append(' '.join(desc))

# python 에서 집합이란? 
# - 중복된 것은 들어갈 수 없다. 유니크 값만 lex 에 넣는다.      
lex = set() # filckr 데이터 셋에서 사용하는 단어들
for key in lookup:
  [lex.update(d.split()) for d in lookup[key]]

# print(lookup) # dic 형태 확인하기
...
# 훈련과 테스트 데이터로 데이터셋이 이미 구분돼어 있다.
# Warning, running this too soon on GDrive can sometimes not work.
# Just rerun if len(img) = 0
# join 은 연결해주는 붙여주는 역할
img = glob.glob(os.path.join(root_captioning,'/content/drive/MyDrive/Colab Notebooks/data/ImageCaptioning/Flickr_image/Flicker8k_Dataset', '*.jpg'))
...
def encodeImage(img):
  ...
  # encoding network)
  img = img.resize((WIDTH, HEIGHT), Image.ANTIALIAS) # 사진 크기 299*299 크기로 리사이징
  # Convert a PIL image to a numpy array
  x = tensorflow.keras.preprocessing.image.img_to_array(img) # 이미지를 np.array 로 변경
  # Expand to 2D array (299,299,3)
  x = np.expand_dims(x, axis=0) # 변경된 데이터에 샘플수를 표현하는 차원 추가 (x, 299, 299, 3)
  # Perform any preprocessing needed by InceptionV3 or others
  x = preprocess_input(x) # 픽셀을 -1 ~ 1사이 값으로 스케일링! 모든 모델마다 preprocessing 함수가 존재한다!
  # Call InceptionV3 (or other) to extract the smaller feature set for 
  # the image.
  x = encode_model.predict(x) # Get the encoding vector for the image 299*299 크기의 이미지를 inceptionV3모델에 넣어 feature map 생성
...
# 훈련과 테스트 데이터를 특성맵화 한 후 pkl 확장자로 저장
train_path = os.path.join(root_captioning,"data",f'train{OUTPUT_DIM}.pkl')
if not os.path.exists(train_path):
  start = time()
  encoding_train = {}
  for id in tqdm(train_img):
    image_path = os.path.join(root_captioning,"Flickr_image",'Flicker8k_Dataset', id)
...
# 사용하기 편하게 두 개의 dic 으로 분류
idxtoword = {} # key 값 index,  value 값이 단어
wordtoidx = {} # key 값 단어, value 값이 index
...
# 문장의 시작과 끝에 추가했던 문장에 한 번 더 시작과 끝에 문장이 들어가서 +2 
max_length +=2
print(max_length)
    ...
    for key, desc_list in descriptions.items():
      ...
      for desc in desc_list:
        ...
        for i in range(1, len(seq)):
          ...
          in_seq = pad_sequences([in_seq], maxlen=max_length)[0] # 패딩을 넣어서 모든 길이를 34로 맞춘다.
...
# 모델 설계
# 사진 input
inputs1 = Input(shape=(OUTPUT_DIM,)) # 2048
fe1 = Dropout(0.5)(inputs1) # Dropout 을 사용해서 연결
fe2 = Dense(256, activation='relu')(fe1) # fe1 연결

# 문장 input
inputs2 = Input(shape=(max_length,)) # 34
se1 = Embedding(vocab_size, embedding_dim, mask_zero=True)(inputs2) # Embedding 
se2 = Dropout(0.5)(se1)
se3 = LSTM(256)(se2)

decoder1 = add([fe2, se3]) # 분류기
decoder2 = Dense(256, activation='relu')(decoder1)  
outputs = Dense(vocab_size, activation='softmax')(decoder2) # 다중 분류 1652개 중에서~ 뭐가 나올까?
...
caption_model.layers[2].set_weights([embedding_matrix]) # 전이학습을 위한 셋팅  
caption_model.layers[2].trainable = False # 전이학습을 위해 학습을 막는다
caption_model.compile(loss='categorical_crossentropy', optimizer='adam')

"""### Train the Neural Network"""

# 학습 시작!!!!
number_pics_per_bath = 3 # 3장씩 들어간다.
...
if not os.path.exists(model_path):
  for i in tqdm(range(EPOCHS*2)):
      ...
      caption_model.fit_generator(generator, epochs=1, # 기본 러닝 레이트
                    steps_per_epoch=steps, verbose=1)

# 중간에 나눠서 lr 을 개선하여 하도록 설정
  ...
  for i in range(EPOCHS):
      generator = data_generator(train_descriptions, encoding_train, 
                    wordtoidx, max_length, number_pics_per_bath)
      caption_model.fit_generator(generator, epochs=1,  # 개선된 러닝레이트 사용
                            steps_per_epoch=steps, verbose=1)  
